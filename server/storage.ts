...
export class MemStorage implements IStorage {
  ...
  async updateProduct(id: string, productData: any): Promise<Product> {
    ...
    const updatedProduct = { ...existingProduct, ...productData, updatedAt: new Date().toISOString() };
  ...
  // Cart methods
  async addToCart(cartItemData: any): Promise<Cart> {
    // Check if item already exists in cart
    for (const item of this.carts.values()) {
      if (item.userId === cartItemData.userId && item.productId === cartItemData.productId) {
    ...
    this.carts.set(cartItem.id, cartItem);
    return cartItem;
  }

  async getCartItems(userId: string): Promise<Cart[]> {
    return Array.from(this.carts.values()).filter(item => item.userId === userId);
  }

  async updateCartItem(cartItemId: string, quantity: number): Promise<Cart> {
    const existingItem = this.carts.get(cartItemId);
    if (!existingItem) {
      throw new Error("Cart item not found");
    }
    existingItem.quantity = quantity;
    this.carts.set(cartItemId, existingItem);
    return existingItem;
  }

  async removeFromCart(cartItemId: string): Promise<void> {
    this.carts.delete(cartItemId);
  }

  // Wishlist methods
  async addToWishlist(wishlistItemData: any): Promise<Wishlist> {
    // Check if item already exists in wishlist
    for (const item of this.wishlists.values()) {
      if (item.userId === wishlistItemData.userId && item.productId === wishlistItemData.productId) {
    ...
    this.wishlists.set(wishlistItem.id, wishlistItem);
    return wishlistItem;
  }
  ...
  async createReview(reviewData: any): Promise<Review> {
    ...
    const product = await this.getProductById(reviewData.productId);
    if (product) {
  ...
  async getStats(): Promise<{
    ...
    const activeStores = Array.from(this.users.values())
      .filter(user => user.role === 'vendor').length;
